...
## Bugs connus (résolus) — terrain Kenney + fantôme

Après Phase 1, trois régressions liées au terrain `Group` et au culling ont été corrigées (fantôme figé sur bâtiments Kenney, zones sans pick, lag souris). Détail complet : [`bug/placement-ghost-kenney-frustum-pick.md`](./bug/placement-ghost-kenney-frustum-pick.md).

---

## Contrat couleur herbe (`displayColor`)

**Une seule teinte** pour tuiles Kenney, sol infini, brouillard et tokens CSS :

| Token | Rôle |
|-------|------|
| `terrainCatalog['nature:ground_grass'].displayColor` | source de vérité (`0x2fe7c5` ≈ `#36E6C6` à l’écran) |
| `resolveTerrainDisplayColorHex/Css()` | Three.js + DOM |
| `--terrain-grass-color` | CSS (injecté au boot) |
| `SCENE_FOG_COLOR` | blend automatique herbe + ciel |

**Ne pas** utiliser le baseColor GLB (`~#74ECDD`) — rupture visible avec le horizon.  
Bug et correctif : [`bug/terrain-grass-color-mismatch.md`](./bug/terrain-grass-color-mismatch.md).

---

## Island shore (désactivé)

Le rivage Kenney (falaises, plage, masque organique) est **désactivé** (`islandBeachBorderEnabled: false`).
